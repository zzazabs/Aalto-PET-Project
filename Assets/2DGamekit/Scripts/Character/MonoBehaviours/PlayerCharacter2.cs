...
namespace Gamekit2D
{
	...
	{
		static protected PlayerCharacter2 s_PlayerInstance;
		static public PlayerCharacter2 PlayerInstance { get { return s_PlayerInstance; } }
		...
		void Start()
		{
			...
			SceneLinkedSMB<PlayerCharacter2>.Initialise(m_Animator, this);
		...
		void Update()
		{
			if (PlayerInput2.Instance.Pause.Down)
			{
				if (!m_InPause)
				{
					...
					PlayerInput2.Instance.ReleaseControl(false);
					PlayerInput2.Instance.Pause.GainControl();
					m_InPause = true;
					Time.timeScale = 0;
		...
		protected IEnumerator UnpauseCoroutine()
		{
			...
			PlayerInput2.Instance.GainControl();
		...
		protected void UpdateCameraFollowTargetPosition()
		{
			...
			if (moveVertically)
			{
				float desiredLocalPosY = PlayerInput2.Instance.Vertical.Value * cameraVerticalInputOffset;
		...
		{
			bool faceLeft = PlayerInput2.Instance.Horizontal.Value < 0f;
			bool faceRight = PlayerInput2.Instance.Horizontal.Value > 0f;
		...
		{
			float desiredSpeed = useInput ? PlayerInput2.Instance.Horizontal.Value * maxSpeed * speedScale : 0f;
			float acceleration = useInput && PlayerInput2.Instance.Horizontal.ReceivingInput ? groundAcceleration : groundDeceleration;
			m_MoveVector.x = Mathf.MoveTowards(m_MoveVector.x, desiredSpeed, acceleration * Time.deltaTime);
		}

		public void CheckForCrouching()
		{
			m_Animator.SetBool(m_HashCrouchingPara, PlayerInput2.Instance.Vertical.Value < 0f);
		}
		...
		public void CheckForPushing()
		{
			...
			{
				bool movingRight = PlayerInput2.Instance.Horizontal.Value > float.Epsilon;
				bool movingLeft = PlayerInput2.Instance.Horizontal.Value < -float.Epsilon;
		...
		{
			if (!PlayerInput2.Instance.Jump.Held && m_MoveVector.y > 0.0f)
			{
		...
		{
			float desiredSpeed = PlayerInput2.Instance.Horizontal.Value * maxSpeed;

			float acceleration;

			if (PlayerInput2.Instance.Horizontal.ReceivingInput)
		...
		{
			return PlayerInput2.Instance.Jump.Down;
		}

		public bool CheckForFallInput()
		{
			return PlayerInput2.Instance.Vertical.Value < -float.Epsilon && PlayerInput2.Instance.Jump.Down;
		}
		...
		public bool CheckForHoldingGun()
		{
			...
			if (PlayerInput2.Instance.RangedAttack.Held)
			{
				holdingGun = true;
		...
		{
			if (PlayerInput2.Instance.RangedAttack.Held && m_Animator.GetBool(m_HashHoldingGunPara))
			{
				if (m_ShootingCoroutine == null)
					m_ShootingCoroutine = StartCoroutine(Shoot());
			}

			if ((PlayerInput2.Instance.RangedAttack.Up || !m_Animator.GetBool(m_HashHoldingGunPara)) && m_ShootingCoroutine != null)
			{
		...
		public void OnHurt(Damager damager, Damageable damageable)
		{
			...
			if (!PlayerInput2.Instance.HaveControl)
				return;
		...
		{
			PlayerInput2.Instance.ReleaseControl(true);
		...
		{
			return PlayerInput2.Instance.MeleeAttack.Down;
		}
